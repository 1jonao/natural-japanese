...
日本語の本文を含む文書を成果物として新規作成、編集、リライト、校正するときは、ユーザーがこのスキルを明示していなくても自動的に適用する。Markdown、HTML、LaTeX、プレーンテキストなど、保存形式ではなく日本語本文の有無で判断する。コード、設定値、表の数値、Markdown記法などの機械的な変更だけで、日本語本文を執筆・変更しない作業には適用しない。

自動適用時は既定でクイックモードを使い、完成を報告する前に `lint.py` を必ず実行する。依存関係やキャッシュ権限の問題で最初の実行に失敗した場合は、後述の書き込み可能なキャッシュ先を指定して再実行する。`uv` 自体が存在しない、依存関係の取得に必要な許可が得られないなど、再実行しても検査不能な場合に限り、理由と未実施であることをユーザーへ明示して手動確認へ切り替える。
...
```
UV_CACHE_DIR="${TMPDIR:-/tmp}/natural-japanese-uv-cache" uv run <skill-root>/scripts/lint.py --json <file>
```

`<skill-root>` はこの `SKILL.md` が置かれたディレクトリの絶対パスに置き換える。Codexの制限環境では、既定の `~/.cache/uv` が読み書きできないことがあるため、上記の書き込み可能なキャッシュ先を常に指定する。初回だけ `sudachipy` と `sudachidict-core` の取得が必要になる場合がある。以後、`outline.py`、`terms.py`、`semantic.py` を実行するときも同じ `UV_CACHE_DIR` と `<skill-root>` を使う。
